...
RSpec.describe "old_articles/new", :type => :view do
  before(:each) do
    assign(:old_article, OldArticle.new(
      :guid => "MyString",
      :locale => 1,
      :preview => false,
      :section_id => 1,
  ...
  it "renders new old_article form" do
    render

    assert_select "form[action=?][method=?]", old_articles_path, "post" do
