...
RSpec.describe "old_sounds/new", :type => :view do
  before(:each) do
    assign(:old_sound, OldSound.new(
      :guid => "MyString",
      :locale => 1,
      :preview => false,
      :title => "MyString",
  ...
  it "renders new old_sound form" do
    render

    assert_select "form[action=?][method=?]", old_sounds_path, "post" do
