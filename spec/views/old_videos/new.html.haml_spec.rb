...
RSpec.describe "old_videos/new", :type => :view do
  before(:each) do
    assign(:old_video, OldVideo.new(
      :guid => "MyString",
      :locale => 1,
      :preview => false,
      :title => "MyString",
  ...
  it "renders new old_video form" do
    render

    assert_select "form[action=?][method=?]", old_videos_path, "post" do
